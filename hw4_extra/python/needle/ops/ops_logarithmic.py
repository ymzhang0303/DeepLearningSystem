...
class LogSumExp(TensorOp):
    ...
    def compute(self, Z):
        ...
        if self.axes:  
            ...
            max_z_reshape = array_api.broadcast_to(max_z.reshape([1] * len(Z.shape)), Z.shape)
            return array_api.log(array_api.sum(array_api.exp(Z - max_z_reshape), axis=self.axes)) + max_z   
        ### END YOUR SOLUTION
