...
class Conv(TensorOp):
    ...
    def compute(self, A, B):
        ...
        _A = A.as_strided(shape=(N, out_H, out_W, K, K, C_in), strides=(Ns, Hs * self.stride, Ws * self.stride, Hs, Ws, Cs))\
                        .compact().reshape((N * out_H * out_W, inner_dim))
        out = _A @ B.compact().reshape((inner_dim, C_out))
