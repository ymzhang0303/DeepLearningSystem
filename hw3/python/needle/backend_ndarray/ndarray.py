...
class NDArray:
    ...
    def __getitem__(self, idxs):
        ...
        for i in range(len(idxs)):
            new_shape.append(int((idxs[i].stop -idxs[i].start - 1) / idxs[i].step + 1))
            new_strides.append(self._strides[i] * idxs[i].step)
            new_offset += idxs[i].start * self._strides[i]
            print("idx start : %d, idx stop : %d, idx step : %d" % (idxs[i].start, idxs[i].stop, idxs[i].step))
        print(new_shape)
        return NDArray.make(
            ...
            device=self._device,
            handle=self._handle,
            offset=self._offset + new_offset,
        )
        ### END YOUR SOLUTION
